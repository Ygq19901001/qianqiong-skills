...
evolution:
  tier: E1
changelog: |
  v2.3.3: 调整 DOI 策略为按目标格式规范保留（APA 7th / GB/T 7714 默认保留 DOI），联网补全需标注来源并提示核实。
  v2.3.0: 重新释放技能，包含完整场景路由/互斥声明/质量标准/诊断系统/联网规则/自进化体系。
  v1.3.0: 格式化前输出变更报告给用户确认，确认后才生成文档。
  v1.2.0: 新增自定义格式上传支持。
  v1.1.0: 中英文分节排序、标题不加粗、每条文献独立分段、默认去除DOI（已废止，详见 v2.3.3）。
  v1.0.1: 默认输出 .docx。
  v1.0.0: 初始版本。
---
...
**Q1：文献类型识别不了怎么办？**
保留原文 + 标记 ⚠️ 需人工确认（QP-E801），不瞎猜类型。用户可手动指定（期刊/图书/章节/会议/学位论文/网页）。

**Q2：作者或年份缺失？**
留空或标 Author unknown，不编造（QP-E802）。联网补全的 DOI/信息必须标注来源并提示用户二次核实。

**Q3：pandoc 没装怎么办？**
降级输出 .md 并告知原因（QP-E803），不中断任务。

**Q4：要的格式不在五种标准里？**
支持 APA 7th/6th、GB/T 7714-2015、MLA 9th、Chicago + 自定义格式。超出范围按自定义处理或说明（QP-E804）。

**Q5：中英文文献怎么排序？**
中文文献在前（拼音/笔画排序），英文在后（字母序），分节列出。
...
| 码 | 含义 | 修正引导 |
|----|------|---------|
| QP-E801 | 文献类型无法识别 | 保留原文 + ⚠️ 标记，请求用户确认 |
| QP-E802 | 文献信息缺失 | 留空/标 Author unknown，不编造 |
| QP-E803 | pandoc 不可用 | 降级 .md 输出 + 说明原因 |
| QP-E804 | 格式超出支持范围 | 按自定义格式处理或说明限制 |
| QP-E805 | 联网补全待核实 | 标注来源，提示用户二次核实 |

## 输入/输出约束

| 项 | 约束 |
|----|------|
| 输入 | 原始文献（可混格式粘贴）+ 目标格式（默认 APA 7th） |
| 输出 | .docx（默认）/ .md / .txt + 变更报告（先确认后生成） |
| 文献类型 | 期刊/图书/章节/会议/学位论文/网页 6 类 |
| 格式 | APA 7th/6th、GB/T 7714-2015、MLA 9th、Chicago、自定义 |
| 铁律 | 不编造文献、不伪造 DOI、不自动发布 |

## 变更记录（ADR）

- 2026-06-23: v2.3.3 调整 DOI 策略：APA 7th / GB/T 7714 默认保留 DOI；联网补全 DOI 必须标注来源并提示用户二次核实。
- 2026-06-23: v2.0.0 初始发布；v2.3.0 补充示例、画像、ADR与触发词。
